                        ...
                        <div class="info">
                            ...
                            <h4>
                                <a href="event-details.html">
                                    {{ $ticket['title'] ?? 'Event Ticket Registration' }}
                                </a>
                            </h4>
                            
                            <!-- Dynamic Date Output with SVG Map Component -->
                            @if(!empty($ticket['date']))
                                <p>
                                    ...
                                    <strong class="ms-2 text-white">{{ \Carbon\Carbon::parse($ticket['date'])->format('M d, Y') }}</strong>
                                </p>
                            @endif

                            <!-- Dynamic Time Output with SVG Clock Component -->
                            @if(!empty($ticket['time']))
                                <p>
                                    <span>
                                    ...
                                    <strong class="ms-2 text-white">{{ \Carbon\Carbon::parse($ticket['time'])->format('h:i A') }}</strong>
                                </p>
                            @endif

                            <!-- Dynamic Location Output with SVG Pin Component -->
                            @if(!empty($ticket['location']))
                                <p>
                                    ...
                                    <strong class="ms-2 text-white">{{ $ticket['location'] }}</strong>
                                </p>
                            @endif
                        </div>

                        <!-- Dynamic Image Rendering with Fallback and Custom CSS Constraints -->
                        <img decoding="async" 
                             src="{{ !empty($ticket['image_path']) ? asset($ticket['image_path']) : asset('images/local/default_placeholder.jpg') }}" 
                             style="width:100%; height:500px; object-fit:cover; object-position: center;"
                             alt="{{ $ticket['title'] ?? 'Running Event' }}">
                    </div>

                    <!-- Pricing & Buy Now Interactive Module -->
                    <div class="tf-info-price">
                        <h4>Ticket</h4>
                        <p class="price">
    <span>
        @if(!empty($ticket['price']) && is_numeric(str_replace([',', ' '], '', $ticket['price'])))
            {{ number_format((float) str_replace([',', ' '], '', $ticket['price'])) }} MMKs
        @else
        ...
        @endif
            ...
            @empty
                <!-- Fallback Container if No Active Tickets Exist in the Database -->
                <div class="text-center py-5 w-100">
                    <h3 class="text-muted">No scheduled event registration packages available right now.</h3>
                </div>
            @endforelse
        </div>
    </div>
