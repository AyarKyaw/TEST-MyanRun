...
<head>
    ...
    <title>Admin Portal - MYAN RUN</title>
...
<body class="font-sans flex items-center justify-center min-h-screen relative" 
    ...
    <div class="max-w-md w-full bg-white/95 p-10 rounded-2xl shadow-2xl border border-slate-200 mx-4 backdrop-blur-sm">
        ...
        <form id="loginForm" class="space-y-6" action="{{ route('admin.login') }}" method="POST">
            ...
            <div>
                <label class="block text-xs font-bold uppercase tracking-wider text-slate-600 mb-2">Admin Email</label>
                <div class="relative">
                    <span class="absolute inset-y-0 left-0 pl-3 flex items-center text-slate-400">
                        <i class="fas fa-user-shield"></i>
                    </span>
                    <input type="email" name="email" value="{{ old('email') }}" placeholder="admin@myanrun.com" required 
            ...
            <div class="pt-6 border-t border-slate-100 text-center">
                <p class="text-[11px] text-slate-400 uppercase tracking-tighter">
                    &copy; 2026 MYAN RUN Management System. <br> All activities are logged.
                </p>
            </div>
        </form>
    </div>
